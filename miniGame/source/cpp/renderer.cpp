...
HRESULT CRenderer::Init(HWND hWnd, bool bWindow) {
	...
	d3dpp.AutoDepthStencilFormat = D3DFMT_D24S8;			// 4ビットZバッファ8ビットステンシルバッファ作成
	...
	if (FAILED(hr) && pErrMessage != nullptr) {
		//エラーメッセージ表示
...
void CRenderer::Uninit(void) {
	//エフェクト終了
	if (m_pEffect != nullptr) m_pEffect->End();
...
void CRenderer::BeginPassEffect(DWORD dwPassFlag) {
	...
	if (m_pEffect != nullptr) m_pEffect->BeginPass((int)typePass);
}
...
void CRenderer::EndPassEffect(void) {
	if (m_pEffect != nullptr) m_pEffect->EndPass();
}

//シェーダのワールドマトリックスを設定
void CRenderer::SetEffectMatrixWorld(D3DXMATRIX mtxWorld) {
	if (m_pEffect != nullptr) m_pEffect->SetMatrix("g_mWorld", &mtxWorld);
}

//シェーダのビューマトリックスを設定
void CRenderer::SetEffectMatrixView(D3DXMATRIX mtxView) {
	if (m_pEffect != nullptr) m_pEffect->SetMatrix("g_mView", &mtxView);
}
...
void CRenderer::SetEffectTextureShadowMap(LPDIRECT3DTEXTURE9 tex) {
	if (m_pEffect != nullptr) m_pEffect->SetTexture("g_texShadowMap", tex);
}

//シェーダのライトを設定
void CRenderer::SetEffectLightVector(D3DXVECTOR4 vecLight) {
	if (m_pEffect != nullptr) m_pEffect->SetVector("g_Light", &vecLight);
}

//シェーダのライトのビューマトリックスを設定
void CRenderer::SetEffectLightMatrixView(D3DXMATRIX mtxView) {
	if (m_pEffect != nullptr) m_pEffect->SetMatrix("g_mLightView", &mtxView);
}

//シェーダのライトのプロジェクションマトリックスを設定
void CRenderer::SetEffectLightMatrixProj(D3DXMATRIX mtxProj) {
	if (m_pEffect != nullptr) m_pEffect->SetMatrix("g_mLightProj", &mtxProj);
}

//シェーダの視点を設定
void CRenderer::SetEffectPosView(D3DXVECTOR4 posV) {
	if (m_pEffect != nullptr) m_pEffect->SetVector("g_posEye", &posV);
}
...
void CRenderer::SetEffectFogColor(D3DXCOLOR colFog) {
	...
	if (m_pEffect != nullptr) m_pEffect->SetVector("g_fogColor", &vecColor);
}
...
void CRenderer::SetEffectFogRange(float fFogStart, float fFogEnd) {
	...
	if (m_pEffect != nullptr) m_pEffect->SetFloatArray("g_fogRange", afFogRagne, 2);
}

//シェーダのマテリアルのディフューズ色を設定
void CRenderer::SetEffectMaterialDiffuse(D3DXCOLOR matDiffuse) {
	if (m_pEffect != nullptr) m_pEffect->SetVector("g_matDiffuse", &D3DXVECTOR4(matDiffuse.r, matDiffuse.g, matDiffuse.b, matDiffuse.a));
}

//シェーダのマテリアルのエミッシブ色を設定
void CRenderer::SetEffectMaterialEmissive(D3DXCOLOR matEmissive) {
	if (m_pEffect != nullptr) m_pEffect->SetVector("g_matEmissive", &D3DXVECTOR4(matEmissive.r, matEmissive.g, matEmissive.b, matEmissive.a));
}

//シェーダのマテリアルのスペキュラーを設定
void CRenderer::SetEffectMaterialSpecular(D3DXCOLOR matSpecular) {
	if (m_pEffect != nullptr) m_pEffect->SetVector("g_matSpecular", &D3DXVECTOR4(matSpecular.r, matSpecular.g, matSpecular.b, matSpecular.a));
}
